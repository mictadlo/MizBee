...
class SpecializedFloatTable extends FloatTable {

  SpecializedFloatTable(String filename) {
    String[] rows = loadStrings(filename);
  ...
  void scrubText(String[] array) {
    for (int i = 0; i < array.length; i++) {
      if (array[i].length() > 2) {
        // remove quotes at start and end, if present
        if (array[i].startsWith("alt") ) {
          array[i] = array[i].substring(16, 18);
        }
      }
    }
  }
...
class SpecializedFloatTableTwo extends FloatTable {

  SpecializedFloatTableTwo(String filename) {
    String[] rows = loadStrings(filename);
  ...
  void scrubText(String[] array) {
    for (int i = 0; i < array.length; i++) {
      if (array[i].length() > 1) {
        ...
        else if (array[i].startsWith("RO3G_") ) {
          if ( !array[i].endsWith(".1") ) {
            println( array[i] );
          }
          array[i] = array[i].substring(5,10);
